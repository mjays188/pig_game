...
func (s *Series)GetEventResultTsp1(tsp1 int) (string, error) {
    ...
    drawPercentage := (float64(draws*100)/float64(totalGames))
    resultString = fmt.Sprintf("Result: Wins, losses and draws staying at k = %d : %v/%v(%.2f%%) , %v/%v(%.2f%%) , %v/%v(%.2f%%) \n", tsp1, p1Wins, totalGames, p1WinPercentage, p2Wins, totalGames, p2WinPercentage, draws, totalGames, drawPercentage) 
